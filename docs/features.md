...
- **SEP-38 Anchor RFQ (fiat/crypto quotes)** (feat/full-roadmap-integration) — Implemented the first slice of the fiat on/off-ramp layer. Added `apps/web/src/server/sep38.service.ts` (`getPrices`, `requestQuote`, `getQuote`) talking to a configurable anchor (`SEP38_ANCHOR_URL` / `SEP38_API_KEY`): indicative prices via `GET /api/sep38/prices` (Redis-cached, 30s) and firm, expiring quotes via `POST /api/sep38/quote` (ADMIN-gated, bearer-authenticated to the anchor, persisted). Added `GET /api/sep38/quote/[id]` to retrieve a quote. Introduced a new `RampQuote` Prisma model + migration (firm-quote audit trail and the reuse point Circle/MoneyGram will attach payouts to) and a shared `apps/web/src/lib/assets.ts` helper converting Zelyo's `{ code, issuer }` refs to/from SEP-38 asset ids (`stellar:native` / `stellar:CODE:ISSUER` / `iso4217:USD`). Added the `sep38` rate limiter, audit logging (`SEP38_GET_PRICES` / `SEP38_POST_QUOTE` / `SEP38_GET_QUOTE`), env schema, and unit tests (asset helper, service with mocked anchor, and all three routes). Anchor integration is mock-backed until a partner is chosen; Anchor Platform, Circle, and MoneyGram remain as follow-up slices that reuse `assets.ts` + `RampQuote`.

- **OpenZeppelin Stellar Channels fee sponsorship** (feat/full-roadmap-integration) — Migrated from the decommissioned Launchtube service to OpenZeppelin Stellar Channels for gasless verify/claim flows. Created `apps/web/src/lib/channels.ts` client (`submitSponsored`) using `@openzeppelin/relayer-plugin-channels` to submit signed transaction XDRs through a channel-account relayer. Added `USE_CHANNELS`, `CHANNELS_URL`, and `CHANNELS_API_KEY` env vars. Extended `lib/stellar.ts` submission helpers (`submitRegister`, `submitVerifyAndRegister`, `publishRoot`, `issueSorobanAsset`, `setVerifiedFlag`, `issuePayment`, `issueClaimableBalance`) with an optional `sponsor: "channels"` mode. Wired `verification.service.ts` and `jobgate.service.ts` to pass the sponsor flag when `USE_CHANNELS=true`. Added unit tests for the Channels client and sponsor paths in `channels.test.ts`, `stellar.test.ts`, `stellar-soroban.test.ts`, `stellar-rewards.test.ts`, `verification.service.test.ts`, and `jobgate.service.test.ts`. Default behavior (direct RPC/Horizon submission) is unchanged when Channels is disabled.

- **SEP-10 Web Authentication + passkey-kit Smart Wallets (SEP-45)** (feat/sep10-passkey-sep45) — Implemented wallet linking for holders. Added a `HolderWallet` Prisma model separate from the ZK identity commitment (`HolderKey`) so one holder can have both a SEP-10 Stellar account and a passkey-backed Soroban smart wallet. Added `GET /api/sep10/challenge` and `POST /api/sep10/token` to prove ownership of a Stellar `G...` account, with server-signed challenge transactions, client-signature verification, Redis nonce replay protection, and a SEP-10 JWT. Added `passkey-kit` client helper at `lib/passkey.client.ts` to register/connect WebAuthn smart wallets and sign transactions off-chain. Added `GET/PUT/DELETE /api/holder/wallet` for CRUD on linked wallets. Updated `/api/verify` to accept `C...` smart-wallet addresses as `boundStellarAddress`. Extended `/wallet/keys` (`WalletLinker` component) with UI to connect a Stellar wallet or create a passkey wallet. Added rate limiters (`sep10`, `holderWallet`), audit logging, env schema validation, and unit tests for the service, routes, and client helper. On-chain SEP-45 signature verification remains disabled until Soroban host functions are available; smart wallets work via Path B (server attestor signs `CredentialRegistry.register`). **Also fixed reward delivery to `C...` smart wallets:** `claimGate` now detects contract destinations and routes `CLAIMABLE_BALANCE` / `REGULATED_ASSET` rewards through the asset's Stellar Asset Contract (`mint` for issuer-issued assets, `transfer` for others) instead of classic Horizon `payment` / `createClaimableBalance` operations, which only accept `G...` destinations.

- **SEP-8 Token-Gated Rewards & Regulated Asset Approval Server** (feat/sep8-token-gated-rewards) — Implemented Task 10.4: a SEP-8 approval server at `POST /api/sep8/approve` that co-signs regulated-asset payment transactions when the destination has a Zelyo `VERIFIED` proof, and rejects them otherwise. Added a new `REGULATED_ASSET` reward type to `GateForm` and `JobGate` so issuers can offer SEP-8 regulated assets as claimable rewards. Compliance checks use the existing `Verification` DB mirror (no contract redeploy required). Added `sep8` rate limiter, audit logging, and unit tests for the service, route, and job gate dispatch. The existing `FLAG` reward type remains unchanged and is still broken pending a future contract update.
...
- **Increase E2E timeouts for in-browser proving on CI** (develop) — Addressed the GitHub Actions runner timeout bottleneck (Task 7.7) by increasing the Playwright serial test configure timeout in `reveals.spec.ts` to 15 minutes (`900_000` ms) and step timeouts to 5 minutes (`300_000` ms), ensuring stable, non-flaky execution on resource-constrained runners during the in-browser WASM proving and testnet transaction phases.

- **Bind job claims to targeted gate during ZK verification** (develop) — Enforced verification scoping to job gates. The `/api/verify` API handler resolves client-supplied `jobGateSlug` to `jobGateId` and forwards it to `verifyAndRegister` to store in the verification record. The `claimGate` service enforces that the verification was either verified without a targeted gate context or targeted the specific gate being claimed. Added unit test coverage in `verify-api.test.ts` and `jobgate.service.test.ts` for verifying slug resolution and claim gate binding.

- **Cryptographic binding of credentialId to ZK proofs** (develop) — Implemented strict validation for client-supplied `credentialId` during proof verification (`verifyAndRegister` service). Validates that the credential exists, is active, has attributes matching all disclosed raw attributes in the public inputs, and its leaf index falls within the Merkle tree boundary defined by the historical root's publication timestamp. Added a comprehensive test suite in `verification.service.test.ts` covering missing/inactive credentials, attribute mismatches, out-of-bounds leaf index checks, and successful validations.

- **Railway deploy readiness fixes** (fix/railway-deploy-readiness) — Hardening pass before first Railway deploy. Added explicit `pg` dependency for the Prisma 7 driver adapter (`@prisma/adapter-pg` peer). Removed `.gitignore` entries so the generated circuit artifacts (`zelyo_credential.json`, `vk`, `vk_hash`, `manifest.json`) are tracked and available in the deployed image without requiring `nargo`/`bb` on the builder. Simplified `railway.json` build command (redundant `prisma generate` removed; `postinstall` + web `build` already run it). Made `/api/health` non-blocking for an external Soroban RPC outage: DB + Redis remain required (`503` if either fails), but RPC is reported as `degraded` with `200` so Railway does not restart the container during testnet hiccups. Cleaned up the leftover `apps/web/src/app/node_modules/.vite` cache.

- **Demo-gates hardening: privacy leak fix, closed claim loop, proof reuse, native-XLM reward fix** (feat/demo-gates-privacy) — Follow-up pass on the gates/privacy work. **Privacy leak (critical):** `PrivacyPanel` previously rendered the plaintext *values* of "private" attributes fetched from the DB, on the unauthenticated `/verify/result/[txHash]` and `/jobs/[slug]` pages — anyone with a public txHash could read `learnerName/grade/courseName/issueDate`. Fixed by removing the credential-attribute fetch entirely; the PRIVATE section now lists field names as `[HIDDEN]` with no value (`verification-read.service.ts` no longer returns `credentialAttributes`). **Closed the claim loop:** "Prove with Zelyo" on `/jobs/[slug]` now carries `?gate=<slug>` → `/wallet` → prove → `ProvePanel` redirects back to `/jobs/<slug>?txHash&nullifier&address`, so the claim button is reachable through normal navigation (previously nothing produced those params). Wallet-first proving still lands on `/verify/result/[txHash]`. **Wallet proof status + reuse:** credential cards show a "Proven ✓ · revealed <attrs>" badge, a "View proof" receipt link, and — when arriving from a gate — a "Use this proof to claim" CTA that reuses a qualifying existing proof (same predicate check `claimGate` enforces) instead of re-proving; a gate-context banner explains the flow. **Multi-predicate claim:** kept the strict AND check but returns a specific "requires disclosing: <attr>" message (a track-only proof can't satisfy `track AND grade`). **Native-XLM reward:** `issueClaimableBalance` used `new Asset("XLM","")` which serialized as a credit asset with an empty issuer and 500'd; now uses `Asset.native()` when the reward has no issuer. **Error surfacing:** `claimGate` wraps reward issuance and throws `REWARD_FAILED` (502) with the Horizon/Soroban `result_codes` instead of a blank 500. `pnpm --filter @zelyo/web typecheck && test` green (59 files, 157 tests).

- **Issuer gate creation UI + Privacy Dashboard + Multi-predicate Gates + Time-limited Gates** (feat/demo-gates-privacy) — Direction 1: Issuer gate creation UI at `/issuer/gates` with `GateForm` for creating gates with multiple predicates (AND logic), reward type configuration, and optional expiration. Direction 2: Privacy Dashboard (`PrivacyPanel`) shows live "what you revealed / what stayed private" at claim time and verification result page — makes the ZK story visceral. Direction 3: Multi-predicate gates — `JobGate.requiredPredicate` renamed to `requiredPredicates` (array), `claimGate` loops over all predicates. Direction 5: Time-limited gates — `JobGate.expiresAt` field with server-side check in `claimGate` returning `GATE_EXPIRED`. Privacy Panel wired into `/jobs/[slug]` and `/verify/result/[txHash]`. `pnpm --filter @zelyo/web typecheck && lint && test` green.
