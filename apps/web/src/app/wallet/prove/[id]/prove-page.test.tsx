...
vi.mock("@/lib/db", () => ({
  db: {
    holderKey: { findUnique: vi.fn() },
    credential: { findFirst: vi.fn() },
    holderWallet: { findMany: vi.fn() },
  },
}));
...
vi.mock("@/components/wallet/ProvePanel", () => ({
  ProvePanel: ({ credential, linkedWallets }: { credential: { id: string }; linkedWallets: unknown[] }) => (
    <div>
      panel:{credential.id}:wallets:{linkedWallets.length}
    </div>
  ),
}));
...
describe("/wallet/prove/[id]", () => {
  ...
  it("renders ProvePanel with the credential + merkle path + linked wallets for the owner", async () => {
    ...
    vi.mocked(db.holderWallet.findMany).mockResolvedValue([
      { id: "w1", type: "STELLAR_ACCOUNT", address: "GABC", isDefault: true },
    ] as never);
    const ui = await ProvePage({ params: Promise.resolve({ id: "c1" }), searchParams: Promise.resolve({}) });
    render(ui);
    expect(screen.getByText("panel:c1:wallets:1")).toBeInTheDocument();
  });
});
