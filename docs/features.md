...
- **Reusable KYC for Anchors (SEP-12)** (develop) — Implemented Task 10.2: a SEP-12 customer API at `/api/sep12/customer` supporting `GET` (status check) and `PUT` (submit a Zelyo verification). Anchors can query whether a Stellar account is KYC-verified and submit a Zelyo `verification_id` to mark the account as `ACCEPTED`. The implementation never stores PII; it only records the Stellar account, memo, SEP-12 status, and the referenced `Verification.id`. Added `Sep12Customer` to the Prisma schema with migration `20260706095912_add_sep12_customer`, plus rate limiting (`limiters.sep12`) and audit logging. Added unit tests in `sep12.service.test.ts` and `route.test.ts` covering NEEDS_INFO/ACCEPTED flows, invalid verifications, and account mismatch rejection.
