...
- **Passkeys & Gasless Transactions (Launchtube)** (develop) — Implemented Task 10.3 wiring. Installed `passkey-kit` and added `apps/web/src/lib/passkey.ts`, a client-side wrapper that registers/connects a SEP-45 style smart wallet via WebAuthn and stores the credential in `localStorage`. Added passkey registration UI to `/wallet/keys` via `KeysManager`. Added `apps/web/src/lib/launchtube.ts` and the server relay `POST /api/launchtube/submit` to submit signed Stellar XDR to Launchtube for fee sponsorship. Updated `claimGate` and the claim route to support a `gasless` mode (`direct` vs `launchtube`), and added a "Claim gasless (via Launchtube)" checkbox to `ClaimPanel`. Added environment variables for passkey-kit and Launchtube to `apps/web/src/lib/env.ts` and `.env.example`. Added unit tests for passkey, Launchtube, the relay route, and the gasless claim path.
